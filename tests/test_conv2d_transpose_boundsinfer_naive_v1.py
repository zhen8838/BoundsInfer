...
def naive_impl_v1(input: np.ndarray, weight: np.ndarray, stride=[1, 1], padding=[0, 0], output_padding=[0, 0], dilation=[1, 1], groups: int = 1):
  """ 
        ...
        for ic in range(G_IC):
          for iy in range(input.shape[2]):
            for ix in range(input.shape[3]):
              ...
              filter_y_end = min(weight.shape[2], (int(out_shape[2]) -
                                                  out_y_origin + dilation[0] - 1) / dilation[0])
              filter_x_start = max(0, (-out_x_origin + dilation[1] - 1) / dilation[1])
              filter_x_end = min(weight.shape[3], (int(out_shape[3]) -
                                out_x_origin + dilation[1] - 1) / dilation[1])
              for ky in range(filter_y_start, filter_y_end):
                for kx in range(filter_x_start, filter_x_end):
                  ...
                  output[batch, g * G_OC + oc, out_y, out_x] += input[batch, g * G_IC + ic, iy, ix] * weight[g * G_IC + ic, oc, ky, kx]
  return output
...
def test_torch_naive_v1(in_hw: Tuple[int, int], in_channel: int, output_channel: int, k_hw: Tuple[int, int],
  ...
  output_naive = naive_impl_v1(input, weight, stride, padding, output_padding, dilation, groups)
